...
def main():
    ...
    if st.checkbox("Show Dataset"):
        number = st.number_input("Num,ber of Rows to View",5)
    ...
    #Group
    if st.checkbox("show bar chart group"):
        v_group = data.groupby('Country')
        st.bar_chart(v_group)

    # draft
    st.subheader("Choosing country")
    all_countries_names= list(data['Country'].unique())
    select_countries=st.selectbox("Select country to plot",all_countries_names)
